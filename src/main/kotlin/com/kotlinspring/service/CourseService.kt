...
class CourseService(val courseRepository: CourseRepository) {
    ...
    fun addCourse(courseDTO: CourseDTO): CourseDTO {
        val courseEntity = courseDTO.let { Course(null, it.name, it.category, null) }
    ...
    fun getAllCourses(courseName: String?): List<CourseDTO> {
        val courses = courseName?.let { courseRepository.findCourseByName(it) } ?: courseRepository.findAll()
    ...
    fun updateCourse(courseId: Int, courseDTO: CourseDTO): CourseDTO {
        ...
        return if (existingCourse.isPresent) {
            existingCourse.get().let {
                it.name = courseDTO.name; it.category = courseDTO.category
